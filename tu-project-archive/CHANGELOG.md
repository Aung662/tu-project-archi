...
- **Print / Save-as-PDF (project detail):** a 🖨 Print button and print CSS that strip the site chrome (nav, footer, ads, buttons, purchase panel, media, reviews, similar) and force an ink-friendly light layout — producing a clean, single-column reference sheet of the project metadata, abstract, keywords and citation.
- **Browse search widened:** the `q` query now also searches the abstract (was title + keywords only), so topic-driven searches surface more relevant work.
- Fully internationalized (Burmese-first, English fallback); FE + BE `tsc` EXIT 0; all new pages/endpoints live-verified end-to-end.
...
- **Public statistics, trending & reviews (backend + frontend):**
  - **Schema:** added `Project.viewCount` (+ index) and a new `Review` model (rating 1-5, optional comment, one per user per project). Applied on deploy automatically via `prisma db push` in `deploy:release`.
  - **Public stats page** (`/stats`) — transparent, cached snapshot of the archive: totals (projects/universities/departments/with-file), distributions by year / level / department / university (reusing the inline-SVG `BarList`), and a "Most viewed" list. Backed by a new `GET /api/stats` (aggregations over PUBLISHED projects, 1-min in-process cache).
  - **View counter & Trending** — project detail views increment `viewCount` (best-effort, deduped per IP per day). New `GET /api/projects/trending`; a "🔥 Trending projects" row on the home page and a 👁 view badge on project detail.
  - **Ratings & reviews** — students rate a project 1-5 with an optional comment on the detail page (star picker, average, distribution bars, recent reviews). New `reviews` module: `GET/POST/DELETE /api/reviews/project/:id` (post/delete require auth; one review per user, upserted). All 57 backend tests still pass.
- **Student productivity features (all frontend, no DB change):**
  - **Cite this project** — a "Cite this project" box on every project detail page generates a ready-to-paste reference in **IEEE / APA / MLA** with one-click copy. Author names are parsed from the free-text field and formatted per style (initials, "et al.", etc.). New `src/lib/citation.ts` + `components/CitationBox.tsx`. Logic unit-verified for 1/2/3+ and missing authors.
  - **Recently viewed** — a localStorage-backed strip on the home page shows the visitor's last-viewed projects (no login needed) so they can jump back. New `src/lib/recentlyViewed.ts` + `components/RecentlyViewed.tsx`.
  - **Share** — a Share button on project pages uses the native mobile share sheet (Messenger/Viber/Telegram) and falls back to copy-link on desktop. New `components/ShareButton.tsx`.
  - **Clickable keywords** — keyword chips on a project now link to Browse filtered by that term. Browse reads `?q=` from the URL (wrapped in Suspense for `useSearchParams`).
- **"About this website" page + founder profiles:** new `/about` page tells the project's purpose in flowing, natural English **and** Burmese together (three bilingual mission cards: why it was built, avoiding duplicate titles, built to grow). Below it, a "Meet the team" section with tilt-card profiles for the two founders (photo, name, role, student ID, department, phone/email) driven by `src/lib/founders.ts`. Founder portraits added under `/public/founders/`. Reached via a raised 3D "About this website" button that fills the previously empty cell on the Contact page, plus a Footer link.
- **Speed / latency fixes (fast first load for Myanmar users):** moved the Render API service + database to the **Singapore** region (closest free region to Myanmar; Yangon→Singapore ~40-60ms vs Oregon ~250-350ms). API client now uses an `AbortController` timeout with automatic retry+backoff for GET requests (8s×3), so a sleeping free-tier backend wakes gracefully instead of leaving the UI spinning forever, and shows a clear timeout message on total failure. Keep-alive ping cadence tightened from 10→5 min so the server virtually never goes cold.
