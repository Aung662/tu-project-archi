...
export default function AdminSchools() {
  ...
  function fail(e: unknown) {
    setErr(e instanceof Error ? e.message : t.scActionFailed.my);
    setMsg(null);
  }
  ...
  async function deleteUni(id: string) {
    if (!confirm(t.scConfirmDeleteUni.my)) return;
    try {
  ...
  async function deleteDept(id: string) {
    if (!confirm(t.scConfirmDeleteDept.my)) return;
    try {
  ...
  return (
    <div className="space-y-4">
      ...
      {unis.length === 0 ? (
        <EmptyState title={t.scNoUnis.my} />
      ) : (
        <div className="space-y-4">
          {unis.map((u) => (
            <div key={u.id} className="card p-5">
              <div className="flex flex-wrap items-start justify-between gap-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => deleteUni(u.id)} className="btn-danger px-3 py-1 text-xs">{t.scDelete.my}</button>
                </div>
              </div>
                ...
                {deptForUni === u.id && (
                  <form onSubmit={(e) => saveDept(e, u.id)} className="mb-3 flex flex-wrap items-end gap-2 rounded-lg bg-white/5 p-3">
                    ...
                    <div className="flex-[2]">
                      <label className="label">{t.scDeptName.my} *</label>
                    ...
                    {u.departments.map((d) => (
                      <li key={d.id} className="flex items-center justify-between py-2 text-sm">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button onClick={() => deleteDept(d.id)} className="btn-danger px-3 py-1 text-xs">{t.scDelete.my}</button>
                        </div>
                      </li>
                    ))}
                  </ul>
