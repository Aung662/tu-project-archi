...
export default function LibraryPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-semibold text-slate-100">{tr(t.savedProjects)}</h2>
        {bookmarks.length === 0 ? (
          <EmptyState
            title={tr(t.savedEmptyTitle)}
            hint={tr(t.savedEmptyHint)}
          />
        ) : (
          <>
          {/* Turn saved projects into a ready-to-paste reference list */}
          <BibliographyExport bookmarks={bookmarks} />
